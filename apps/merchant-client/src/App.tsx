...
import { errorRedirectUrl } from './utils/errorRedirectUrl';
...
export const App: React.FC = () => {
  ...
  const [errorText, setErrorText] = React.useState<any | undefined>(undefined);
    ...
    <ModalRoot activeModal={activeModal}>
      ...
      >
        <Div>
          ...
          <FormItem top="Merchant website domain" bottom={inputLink.error}>
            <Input
              type="text"
            ...
            >
              ...
              <Button
                ...
                onClick={() => {
                  createMerchantR({
                    name: input.value,
                    evm_withdraw_address: inputAddress.value,
                    description: inputDesc.value,
                    domain: inputLink.value,
                    tron_withdraw_address: inputTronAddress.value,
                  })
                    .unwrap()
                    .then((res) => {
                      // @ts-ignore
          ...
          <FormItem>
            ...
            >
              ...
              <Button
                size="l"
                stretched
                disabled={inputDel.value !== selectMerhant?.name}
                onClick={() => {
                  gelMerchant();
                }}
              >
      ...
      >
        {selectMerhant ? (
          <Div>
            <FormItem top="Amount invoice" bottom={inputCreateValue.error}>
              <Input
                type="number"
                placeholder="100"
                value={inputCreateValue.value}
                onChange={(e) => inputCreateValue.change(e.target.value)}
              ...
              {assets ? (
                <Select
                  ...
                  onChange={(e) => {
                    if (getAssetsData) {
                      setBlockchain(
                        Object.keys(getAssetsData[e.target.value]).map(
                          (key) => {
                            return {
                              value: key,
                              label: key,
                      ...
                      inputCreatAsset.change(e.target.value);
                    }
                  }}
                />
              ) : null}
            ...
            <FormItem top="Blockchain">
              {blockchain ? (
                <ChipsSelect {...blockchainChipsProps}></ChipsSelect>
              ) : null}
            </FormItem>

            <FormItem top="Description" bottom={inputCreateDesc.error}>
              <Textarea
            ...
            {selectedBlockchain.find((v) => v.value === 'tron') && (
              <FormItem top="Address Tron" bottom={inputTronAddress.error}>
                <Input
                  type="text"
                  placeholder="T..."
                  value={inputTronAddress.value}
                  onChange={(e) => inputTronAddress.change(e.target.value)}
                  status={inputTronAddress.iserr}
                />
              </FormItem>
            )}
              ...
              (v) =>
                ...
                >
                  <Input
                    ...
                    onChange={(e) => inputBitcoinAddress.change(e.target.value)}
            ...
            <FormItem>
              ...
              >
                ...
                <Button
                  ...
                  disabled={
                    !(
                      inputCreateValue.iserr === 'valid' &&
                      inputEvmAddress.iserr === 'valid' &&
                      inputTronAddress.iserr === 'valid' &&
                      (selectedBlockchain.includes({ label: "bitcoin", value: "bitcoin" }) ? inputBitcoinAddress.iserr === 'valid' : true)
                    )
                  }
                  onClick={() => {
                    createInvoice();
                ...
                {webhookHistory.map((webhook, key) => (
                  <tr
                    ...
                    onClick={() => {
                      ...
                      setActiveModal('webhook_history_one');
                    }}
                  >
                    <td>{truncAddress(webhook.payment_id)}</td>
        ...
        header={
          <ModalPageHeader
            after={
              !isDesktop && (
                <PanelHeaderButton
                  onClick={() => setActiveModal('webhook_history')}
                >
                  <Icon24Dismiss />
                </PanelHeaderButton>
              )
        ...
        <Div className="polus-select">
          {webhookHistoryOne ? (
            <div>
                    ...
                    <td>
                      <SubnavigationBar>
                        <SubnavigationButton
                          disabled
                          className={
                            webhookHistoryOne.response_status_code === 200
                              ? 'completed'
                              : 'expired'
                          }
                        >
              ...
              <SimpleCell after={webhookHistoryOne.endpoint} disabled>
                Webhook Url
              </SimpleCell>
              {/* {webhookHistoryOne.error !== '' ? <SimpleCell after={webhookHistoryOne.error} disabled>
                            Error
                        </SimpleCell> : null } */}
              <SimpleCell after={webhookHistoryOne.payment_id} disabled>
                Payment ID
              </SimpleCell>

              <CardGrid size="l">
                <Card style={{ overflowY: 'scroll' }}>
                  <Div style={{ maxHeight: '300px', maxWidth: '100%' }}>
                    <pre>{webhookHistoryOne.response_body}</pre>
                  </Div>
                </Card>
              </CardGrid>
            </div>
                ...
                tabbar={
                  ...
                  auth === 2 && (
                    <Tabbar>
                      ...
                      <TabbarItem text="Log out" onClick={() => logOut()}>
                        <Icon28DoorArrowRightOutline />
                      </TabbarItem>
                    </Tabbar>
                  )
                ...
                <div id="main">
                  ...
                  <Routes>
                      ...
                      element={
                        ...
                        >
                          <Main
                            ...
                            logOut={logOut}
                            setSelectMerchantId={setSelectMerchantId}
                            isDesktop={isDesktop}
                            newApi={newApi}
                      ...
                      element={
                        ...
                        >
                          <Merchant
                            ...
                            setActiveModal={setActiveModal}
                            consoleLog={consoleLog}
                            setSelectMerhant={setSelectMerhant}
                            logOut={logOut}
                            setSelectMerchantId={setSelectMerchantId}
                            isDesktop={isDesktop}
                            getWebhookHistory={getWebhookHistory}
                            newApi={newApi}
                            onErrorApi={onErrorApi}
                          />
                        </View>
                    ...
                    <Route
                      path="/settings"
                      element={
                        <View activePanel={'settings1'} id="view">
                          <Settings
                            id="settings1"
                            setActiveModal={setActiveModal}
                          />
                    ...
                    <Route
                      path="/invoices"
                      element={
                        <View activePanel={'invoices1'} id="view">
                          <Invoices
                            id="invoices1"
                            setActiveModal={setActiveModal}
                          />
              ...
              >
                ...
                <Group>
                  <CardGrid size="l">
                    <Card>
                      <SimpleCell
                        ...
                        after={<Icon28DoorArrowRightOutline />}
                      >
                        {me?.email}
                      </SimpleCell>
                    </Card>
                  ...
                  <Div className="wallet1">
                    ...
                    <iframe
                      src={import.meta.env.VITE_REACT_APP_IFRAME_URL}
                      width="100%"
                      style={walletOpen ? {} : { display: 'none' }}
                      height={'500px'}
                    />
                  </Div>
                  <Div>
