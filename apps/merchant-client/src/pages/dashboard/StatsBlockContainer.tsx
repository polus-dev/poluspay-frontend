...
const StatsBlock = (props: StatsBlockProps) => {
  ...
  // const {staticsBlock} = useMerchantStatistics({merchantId: props.merchantId, fromData: getBeginningOfDate("month").toISOString(), toData: new Date().toISOString()});
  const {staticsBlock} = useMerchantStatistics({merchantId: props.merchantId, fromData: props.fromData, toData: props.toData});
  return (
...
export const StatsBlockContainer = () => {
  ...
  return (
    <div className="dashboard__stats">
      ...
      {selectedMerchant &&  <StatsBlock merchantId={selectedMerchant.id} toData={new Date().toISOString()} fromData={getBeginningOfDate("month").toISOString()} /> }
    </div>
  )
}
