...
import {ErrorBlock} from "@poluspay-frontend/ui";
...
import {DateUnion, getBeginningOfDate} from "../../../../../../tools";

interface DashBoardChartContainerProps {
  merchantId?: string;
}
export const DashBoardChartContainer = (props: DashBoardChartContainerProps) => {
  ...
  const {staticsBlock, isError, isLoading, chartData} = useMerchantStatistics({merchantId: props.merchantId, toData: toData.current, fromData: getBeginningOfDate(fromData).toISOString()})
  ...
  return (
    <div className="dashboard__charts-container">
      <div className="dashboard__charts-container-item">
        ...
        ) : isBarError || chartData?.total_payments_per_day.length === 0 ? (
          <ErrorBlock
            title={chartData?.total_payments_per_day.length === 0 ? emptyTitle : errorTitle}
            hasBackground={false}
          />
        ) : chartData ?  (
          <DashboardBarChart statistics={chartData} fromData={fromData} setFromData={setFromData} />
        ): null}
      </div>
      <div className="dashboard__charts-container-item">
        {isDoughnutLoading ? (
