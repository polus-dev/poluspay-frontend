...
export const useMerchantStatistics = ({merchantId, toData, fromData}: IUseMerchantStatistics) => {
  ...
  const {data: statistics} = useGetMerchantStatisticsQuery({merchant_id: merchantId, from_date: fromData, to_date: toData});
  const [staticsBlock, setStaticsBlock] = useState<[StatsBlock, StatsBlock, StatsBlock, StatsBlock, StatsBlock]>();
  useEffect(() => {
    if (statistics) {
      const amountOfProceeds = statistics.turnover_detalization?.reduce((acc, item) => acc + +item.amount_decimals, 0) || 0;
      const successFullPayments = statistics.success_payments;
      const conversationRate =  successFullPayments / statistics.total_payments * 100 || 0;
