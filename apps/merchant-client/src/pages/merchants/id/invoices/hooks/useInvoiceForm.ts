...
import {BlockchainItem, blockchainList} from "../../../../../components/ui/Wallets/wallet-list";
...
export const useInvoiceForm = (merchantId: string) => {
    ...
    const [availableMerchantAssets, setAvailableMerchantAssets] = useState<AssetRepresentation[]>([]);
    const [showPreviewForm, setShowPreviewForm] = useState(false);
    const [availableAssetNetworks, setAvailableAssetNetworks] = useState<BlockchainItem[]>([]);
    const [selectedAsset, setSelectedAsset] = useState<AssetRepresentation | null>(null);
    const [selectedNetworks, setSelectedNetworks] = useState<BlockchainItem[]>([]);
    const [availableCategories, setAvailableCategories] = useState<string[]>([]);
    const [merchantIsNotAvailableToCreateInvoice, setMerchantIsNotAvailableToCreateInvoice] = useState(false);
    const [createInvoice, {isLoading: isCreateInvoiceLoading}] = useCreatePaymentMutation()
  ...
  useEffect(() => {
    if (merchantWallets && assets) {
      ...
      const availableNetworks = [...new Set(merchantWallets.map(e => e.network))]
      const availableAssets = assets.getAssetsByNetworks(availableNetworks, true);
      setAvailableMerchantAssets(availableAssets)
      setAvailableCategories(Array.from(new Set(availableAssets.map(e => e.categories).flat())));
    }
  ...
  useEffect(() => {
    if (selectedAsset) {
      setAvailableAssetNetworks(selectedAsset.networks.map(e => blockchainList.find(b => b.label === e)!))
      setSelectedNetworks([])
