            ...
            <Div style={{ paddingTop: 0 }}>
              ...
              >
                <h6 className="polus-h6">
                  ...
                  <span style={{ userSelect: 'all' }}> {merchant.id}</span>
                </h6>
              ...
              {typePage === 0 ? (
                <div>
                  ...
                  <FormItem top="DNS TXT Record code" style={{ width: '50%' }}>
                    <Input value={merchant?.domain_confirmation_code} />
                  </FormItem>
                  <Div>
                    ...
                    >
                      ...
                      <LinkVk
                        style={{ color: 'var(--polus_red)', marginTop: '16px' }}
                        onClick={() => props.setActiveModal('delete_merchant')}
                      >
                        Delete merchant
                      </LinkVk>
                    </div>
              ...
              {typePage === 1 ? (
                <div>
                  <FormLayoutGroup
                    mode={props.isDesktop ? 'horizontal' : 'vertical'}
                  >
                  ...
                  >
                    <Input
                      type="text"
                      placeholder={merchant?.evm_withdraw_address ?? ''}
                      value={inputEvmAddress.value}
                      onChange={(e) => {
                        inputEvmAddress.change(e.target.value);
                      }}
                  ...
                  <FormItem top="Success url" bottom={successUrl.error}>
                    <Input
                      type="text"
                      placeholder={merchant?.success_redirect_url ?? ''}
                      value={successUrl.value}
                      onChange={(e) => {
                        successUrl.change(e.target.value);
                      }}
                      status={successUrl.iserr}
                    />
                  </FormItem>

                  <FormItem top="Fail url" bottom={failUrl.error}>
                    <Input
                      type="text"
                      placeholder={merchant?.fail_redirect_url ?? ''}
                      value={failUrl.value}
                      onChange={(e) => {
            ...
            <Group>
              <Div>
                ...
                >
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      props.getWebhookHistory(merchant?.id);
                      props.setActiveModal('webhook_history');
                    }}
                  >
                    WebHook history
                  </Button>
                ...
                >
                  ...
                  >
                    <Input
                      ...
                      onChange={(e) => {
                        inputUrl1.change(e.target.value);
                      }}
                      status={inputUrl1.iserr}
                    />
                  </FormItem>
                  <Button onClick={() => setWebHook()} size="m">
                    Save
                  </Button>
                </FormLayoutGroup>
                ...
                >
                  ...
                  <Button onClick={() => generatedAPIKey()} size="m">
                    Update
                  </Button>
                </FormLayoutGroup>
              </Div>
              ...
              <Div className="mobile-table-2">
                {payments && payments.data.length > 0 ? (
                  <table className="polus-table">
                    ...
                    <tbody>
                      {payments.data.map((invoice, key) => {
                        let asset;
                        let assetAmount;
                        // TODO: REFACTOR
                        if (Object.keys(invoice.assets).length) {
                          const assets = invoice.assets;
                          const blockchain = Object.keys(
                            assets
                          )[0] as Blockchain_t;
                          asset = Object.keys(assets[blockchain])[0] as Asset_t;
                          assetAmount = (
                            assets[blockchain][asset].amount *
                            10 ** -assetsInfo[asset][blockchain].decimals
                          );
                          assetAmount = roundCryptoAmount(assetAmount.toString())
                        } else {
                          asset = assetAmount = "asset didn't set";
                        }
                        return (
                          <tr key={key}>
                            <td>
                              <LinkVk
                                href={`${import.meta.env.VITE_REACT_APP_PAYFORM_URL
                                  }?uuid=${invoice.id}`}
                                target="_blank"
                              >
                                {truncAddress(invoice.id)}
                              </LinkVk>
                            </td>
                            <td>{assetAmount}</td>
                            <td>{asset}</td>
                            <td>
                              {invoice.transaction ? (
                                <LinkVk href={getExplorer(invoice.transaction.network)} target="_blank">
                                  {truncAddress(invoice.transaction.hash)}
                                </LinkVk>
                              ) : (
                                '-'
                              )}
                ...
                {payments &&
                  payments.data.length > 0 && payments.totalCount > limit ? (
                  <div
                    style={{
                      margin: '20px',
                      display: 'flex',
                  ...
                  >
                    ...
                    <Button
                      ...
                      disabled={paymentsIsFetching || offset + limit >= payments.totalCount}
                      size="l"
                      mode="outline"
                    >
                      {paymentsIsFetching ? (
