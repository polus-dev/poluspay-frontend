...
const MerchantWebhookHistoryResp = NewResponse(
    responses.merchant.webhook_history
);
type MerchantWebhookHistoryRespType = typeof MerchantWebhookHistoryResp;
...
class MerchantRepo extends BasicRepo {
    ...
    public async webhook_history(
        data: MerchantWebhookHistory
    ): RRes<MerchantWebhookHistoryRespType> {
        const path = '/api.merchant.webhook_history';
        const resp = await this._client.sendRequest(path, data, true);

        tcheck(MerchantWebhookHistoryResp, resp);
        return resp.data;
    }

    public async regen_key(
        data: MerchantRegenKey
    ): RRes<MerchantRegenKeyRespType> {
