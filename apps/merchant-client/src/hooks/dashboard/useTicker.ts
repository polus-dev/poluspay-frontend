...
export const useTicker = () => {
    ...
    useEffect(() => {
        ...
        priceService.startMainLoop((prices) =>
            setTickers(
                prices.map((price) => ({
                    shift: price.priceChange,
                    name: currencyNameMap[price.symbol].toUpperCase(),
                    fullName: price.symbol,
                    price: Number(price.currentPrice).toFixed(2),
                    shiftPercent: price.priceChangePercent,
                    image: getAssetUrl(
                        import.meta.env.VITE_ASSET_URL,
                        currencyNameMap[price.symbol]
                    ),
                }))
            )
        );
