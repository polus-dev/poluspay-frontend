...
import {useGetPaymentByPaymentIdQuery, useLazyGetPaymentByPaymentIdQuery} from '../store/api/endpoints/payment/Payment';
...
export const usePaymentInfo = (id: string) => {
  ...
  const getInfo = useCallback(async () => {
    ...
    try {
      ...
      const merchant = await getMerchantInfo({merchant_id: payment.merchant_id}).unwrap();
      setInfo({
        payment,
        merchant,
      })
  ...
  useEffect(() => {
    if (currentBlockchain && assets?.getQRCodePaymentNetworks().includes(currentBlockchain)) {
      if (timer.current) clearInterval(timer.current);
      timer.current = setInterval(() => {
        getPaymentInfo({payment_id: id}).unwrap().then((payment) => {
           setStatus(payment.status);
